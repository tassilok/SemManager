...
Public Class clsDBLevel
    ...
    Public Function del_ClassRel(ByVal oList_ClRel As List(Of clsClassRel)) As String()
        ...
        If Not strKeys Is Nothing Then
            If strKeys.Count > 0 Then
                ...
                Try
                    objOPResult = objElConn.Delete(objLocalConfig.Globals.Index, objLocalConfig.Globals.Type_ClassRel, strKeys)
                    objOItem_Result = objLocalConfig.Globals.LState_Success
                Catch ex As Exception
                    objOItem_Result = objLocalConfig.Globals.LState_Error
                End Try
    ...
    Public Function del_Class(ByVal oList_Class As List(Of clsOntologyItem)) As clsOntologyItem
        ...
        objOItem_Result = get_Data_ClassRel(oList_Class, objLocalConfig.Globals.Direction_LeftRight, False, False, False, True)
        If objOItem_Result.Count > 0 Then
            objOItem_Result = objLocalConfig.Globals.LState_Relation
        Else
            objOItem_Result = get_Data_ClassRel(oList_Class, objLocalConfig.Globals.Direction_RightLeft, False, False, False, True)

            If objOItem_Result.Count > 0 Then
                objOItem_Result = objLocalConfig.Globals.LState_Relation
            Else
                objOItem_Result = get_Data_ClassRel(oList_Class, objLocalConfig.Globals.Direction_LeftRight, False, False, True, True)
                ...
                Else
                    ...
                    Else
                        ...
                        For Each objC In objL
                            oList_Objects.Add(New clsOntologyItem(Nothing, Nothing, objC.GUID_Parent, objLocalConfig.Globals.Type_Object))
                        Next
                            ...
                            If Not strACl Is Nothing Then
                                If strACl.Count > 0 Then
                                    Try
                                        ...
                                        objOPResult = objElConn.Delete(objLocalConfig.Globals.Index, objLocalConfig.Globals.Type_Class, strACl)
                                        objOItem_Result = objLocalConfig.Globals.LState_Success
                                    Catch ex As Exception
                                        objOItem_Result = objLocalConfig.Globals.LState_Error
                                    End Try
    ...
    Public Function save_ClassRel(ByVal oList_ClassRel As List(Of clsClassRel)) As clsOntologyItem
        ...
        For Each objOItem In oList_ClassRel
            objOList_Cl_Left.Add(New clsOntologyItem(objOItem.ID_Class_Left, objLocalConfig.Globals.Type_Class))
            objOList_RelType.Add(New clsOntologyItem(objOItem.ID_RelationType, objLocalConfig.Globals.Type_RelationType))
            If Not objOItem.ID_Class_Right Is Nothing Then
                objOList_Cl_Right.Add(New clsOntologyItem(objOItem.ID_Class_Right, objLocalConfig.Globals.Type_Class))
            End If
        Next
        ...
        For Each obj1 In objL1
            ...
            objDict.Add(objLocalConfig.Globals.Field_ID_Class_Right, obj1.objRel.ID_Class_Right)
            ...
            objBulkObjects(l) = New ElasticSearch.Client.Domain.BulkObject(objLocalConfig.Globals.Index, objLocalConfig.Globals.Type_ClassRel, obj1.objRel.ID_Class_Left & obj1.objRel.ID_Class_Right & obj1.objRel.ID_RelationType, objDict)
    ...
    Public Function save_ClassAttType(ByVal oList_ClassAtt As List(Of clsClassAtt)) As clsOntologyItem
        ...
        For Each objOItem In oList_ClassAtt
            oList_Cl.Add(New clsOntologyItem(objOItem.ID_Class, objLocalConfig.Globals.Type_Class))
            oList_AttTyp.Add(New clsOntologyItem(objOItem.ID_AttributeType, objLocalConfig.Globals.Type_AttributeType))
        ...
        For Each objResult In objL
            ...
            objDict = New Dictionary(Of String, Object)
            objDict.Add(objLocalConfig.Globals.Field_ID_AttributeType, objResult.ID_AttributeType)
            objDict.Add(objLocalConfig.Globals.Field_ID_Class, objResult.ID_Class)
            objDict.Add(objLocalConfig.Globals.Field_ID_DataType, objResult.ID_DataType)
            objDict.Add(objLocalConfig.Globals.Field_Min, objResult.Min)
            objDict.Add(objLocalConfig.Globals.Field_Max, objResult.Max)

            objBulkObjects(0) = New ElasticSearch.Client.Domain.BulkObject(objLocalConfig.Globals.Index, objLocalConfig.Globals.Type_ClassAtt, objResult.ID_Class & objResult.ID_AttributeType, objDict)
    ...
    Public Function save_ObjRel(ByVal oList_ObjectRel As List(Of clsObjectRel)) As clsOntologyItem
        ...
        For l = 0 To oList_ObjectRel.Count - 1
            ...
            ReDim Preserve objBulkObjects(l)
            objBulkObjects(l) = New ElasticSearch.Client.Domain.BulkObject(objLocalConfig.Globals.Index, objLocalConfig.Globals.Type_ObjectRel, oList_ObjectRel(l).ID_Object & oList_ObjectRel(l).ID_Other & oList_ObjectRel(l).ID_RelationType, objDict)
        Next
    ...
    End Sub
    Public Function get_Data_Attributes(Optional ByVal oItem_Attribute As clsOntologyItem = Nothing, _
                                        Optional ByVal OList_AttributeTypes As List(Of clsOntologyItem) = Nothing, _
                                        Optional ByVal OList_DataTypes As List(Of clsOntologyItem) = Nothing, _
                                        Optional ByVal doCount As Boolean = False, _
                                        Optional ByVal doClear As Boolean = True) As clsOntologyItem
        ...
        create_BoolQuery_Attributes(oItem_Attribute, OList_DataTypes, OList_AttributeTypes, True)
    ...
    Private Sub create_BoolQuery_Attributes(ByVal objOItem_Attribute As clsOntologyItem, ByVal OList_DataTypes As List(Of clsOntologyItem), ByVal OList_AttributeTypes As List(Of clsOntologyItem), Optional ByVal boolClear As Boolean = True)
        Dim strQuery As String
        ...
        If Not objOItem_Attribute Is Nothing Then

            strQuery = ""

            If objOItem_Attribute.GUID <> "" Then
                strQuery = objOItem_Attribute.GUID
            End If
    ...
    Public Function get_Data_ClassRel(ByVal oList_Class As List(Of clsOntologyItem), _
                                      ByVal Direction As clsOntologyItem, _
    ...
    Public Function get_Data_ObjectAtt(Optional ByRef oItem_Object As clsOntologyItem = Nothing, _
                                       Optional ByVal oItem_AttributeType As clsOntologyItem = Nothing, _
                                       Optional ByVal boolTable As Boolean = False, _
                                       Optional ByVal boolIDs As Boolean = True, _
                                       Optional ByVal doCount As Boolean = False) As clsOntologyItem
        ...
        If doCount = False Then
            If boolIDs = False Then
                ...
                For Each oItem_ObjAtt In oList_ObjAtt
                    get_Data_Attributes(New clsOntologyItem(oItem_ObjAtt.ID_Attribute, objLocalConfig.Globals.Type_Attribute), Nothing, Nothing, False, False)
                Next
    ...
    Public Function get_Data_ObjectRel(ByVal oList_Object As List(Of clsOntologyItem), _
                                       ByVal oList_Other As List(Of clsOntologyItem), _
                                       ByVal oList_RelType As List(Of clsOntologyItem), _
                                       Optional ByVal boolTable As Boolean = False, _
                                       Optional ByVal boolIDs As Boolean = True, _
                                       Optional ByVal doCount As Boolean = False, _
                                       Optional ByVal Direction As String = Nothing, _
                                       Optional ByVal boolClear As Boolean = True) As clsOntologyItem
